...
'''This function applies lexical replacement:'''

def borrowing():
    #call the lexicon list, the onset, nucleus and coda sets, and a list containing potential loanwords
    global lexicon, onset, nucleus, coda, borrowings
    #remove a word from the lexicon
    new_lexicon = random.sample(lexicon, len(lexicon) - 1)
    #get a loanword from the loanword list
    loan = random.choice(borrowings)
    #retrieve the levenshtein distances between the loanword and each of the ohther words in the lexicon
    av_length = []
    for word in new_lexicon:
        ...
        for i, letter in enumerate(word):
            if loan[i] != letter:
                lev += 1
        av_length.append(lev)
    #check if there are words in the lexicon that form a minimal pair with the loanword (levenshtein distance = 1)
    #if there are, add loanword to the lexicon
    if 1 in av_length:
        new_lexicon.append(loan)
        # this prints a line describing the change that happened
        print(rev_consonants[loan[0]] + rev_vowels[loan[1]] + rev_consonants[loan[2]] + ' is borrowed and added to the lexicon.')
        #Update lexicon and sets
        lexicon = new_lexicon
        onset, nucleus, coda = get_onset(lexicon), get_nucleus(lexicon), get_coda(lexicon)
    #if there aren't minimal pairs, skip
    else:
        return
...
def main(file, n_changes, iterations):
    for i in range(int(iterations)):
        ...
        global onset, nucleus, coda, lexicon, borrowings
        ...
        onset, nucleus, coda = get_onset(lexicon), get_nucleus(lexicon), get_coda(lexicon)
        #this line initiates the loanwords list
        borrowings = [random.choice(lexicon)]
        ...
        for n in range(int(n_changes)):
            ...
            if n % 500 == 0:
                ...
                borrowings.append(random.choice(lexicon))
        ...
        #plot consonant frequencies and rank
        freq = Counter()
        for word in lexicon:
            freq[rev_consonants[word[0]]] += 1
            freq[rev_consonants[word[2]]] += 1
        total = sum(freq.values())
        #obtain relative frequencies
        tuples = [(key, value / total) for key, value in freq.most_common()]
        plt.subplot(1, 3, 1)
        plt.plot(range(1, len(tuples) + 1), [tup[1] for tup in tuples])
        plt.gca().set_title('Phonemic frequencies and rank')
        plt.xlabel('Rank')
        plt.ylabel('Frequencies')

        #plot log(frequencies) and log(rank) for consonants
        freq = Counter()
        for word in lexicon:
            freq[rev_consonants[word[0]]] += 1
            freq[rev_consonants[word[2]]] += 1
        ...
        tuples = [(key, value / total) for key, value in freq.most_common()]
        plt.subplot(1, 3, 2)
        plt.plot(np.log(range(1, len(tuples) + 1)), np.log([tup[1] for tup in tuples]))
        plt.xlabel('Log(Rank)')
        plt.ylabel('Log(Frequencies)')
        #find correlation using linear regression
        X = np.array(np.log(range(1, len(tuples) + 1))).reshape(-1, 1)
        model = LinearRegression()
        model.fit(X, np.log([tup[1] for tup in tuples]))
        plt.gca().set_title('Zipf, R^2: ' + str(round(model.score(X, np.log([tup[1] for tup in tuples])), 3)))

        # plot consonants frequencies and log(rank)
        freq = Counter()
        for word in lexicon:
            freq[rev_consonants[word[0]]] += 1
            freq[rev_consonants[word[2]]] += 1
        ...
        tuples = [(key, value / total) for key, value in freq.most_common()]
        plt.subplot(1, 3, 3)
        plt.plot(np.log(range(1, len(tuples) + 1)), [tup[1] for tup in tuples])
        plt.xlabel('Log(Rank)')
        plt.ylabel('Frequencies')
        #find correlation using linear regression
        X = np.array(np.log(range(1, len(tuples) + 1))).reshape(-1, 1)
        model = LinearRegression()
        model.fit(X, [tup[1] for tup in tuples])
        plt.gca().set_title('Yule:, R^2: ' + str(round(model.score(X, [tup[1] for tup in tuples]), 3)))
    plt.show()

...
processed in the form of the 3-dimensional tuple ('d', 'o', 'g'). Then, we can use the two dictionaries to transform 
each word into a integer vector representation through a helper function. The word "dog" will be transformed in 
the tuple (5, 3, 13) for computation purposes.
'''
...
occurs. A wordlist containing the words "dog", "cat", and "pig" will be a represented as the following list:'''
...
contain the onsets, nuclei and codas available in the lexicon in its current state. These three sets are all global variables.
'''
...
print(get_onset(wordlist))
print(get_nucleus(wordlist))
print(get_coda(wordlist))
...
'''
Another helper function that we will need is a function that returns the average Levenshtein distance within a wordlist:
'''
...
def change_onset():
    ...
    new_lexicon = []
    #pick an onset at random, and call it merg_1. This will be the target of the sound change
    merg_1 = random.choice(list(onset))
    #pick an onset at random as the outcome of the sound change
    ...
    for word in lexicon:
        ...
        if word[0] == merg_1:
            ...
            if word[1] in environment:
                #if the nucleus is in the conditioning environment, then change merge_1 into merge_2
...
def change_nucleus():
    ...
    #prepare a new empty list, that will be filled with the form of the words after the sound change apply
    new_lexicon = []
    #pick a nucleus at random, and call it merg_1. This will be the target of the sound change
    merg_1 = random.choice(list(nucleus))
    #pick a nucleus at random as the outcome of the sound change
    ...
    for word in lexicon:
        ...
        if word[1] == merg_1:
            ...
            #if the word does not have merge_1, the change does not apply
            new_lexicon.append(word)
    #this prints a line describing the change that happened
...
def change_coda():
    ...
    # prepare a new empty list, that will be filled with the form of the words after the sound change apply
    new_lexicon = []
    # pick a coda at random, and call it merg_1. This will be the target of the sound change
    merg_1 = random.choice(list(coda))
    # pick a coda at random as the outcome of the sound change
    merg_2 = random.choice(list(rev_consonants))
    # select a random subset of nuclei as the conditioning environment
    environment = random.sample(nucleus, random.randint(0, len(nucleus) - 1))
    #apply the change to the lexicon
...
def main(file, n_changes, iterations):
    for i in range(int(iterations)):
        ...
        #we set initialize lists that will keep track of the number of the iteration, the number of the phonemes,
        ...
        for i in range(int(n_changes)):
            ...
            if i % 500 == 0:
                ...
                #this loop prints the shape of the lexicon before the beginninf of the simulation and after the
